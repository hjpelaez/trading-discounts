...
export default async function CryptoPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
    ...
    const firms = allFirms.filter((f) => {
        const isCrypto = f.categories.includes("crypto");
        ...
        return isCrypto && matchesQuery && matchesPlatform && matchesRating;
    });
